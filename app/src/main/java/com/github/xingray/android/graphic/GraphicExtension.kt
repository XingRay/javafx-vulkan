...
fun calcInnerSize(containerWidth: Int, containerHeight: Int, targetWidthWeight: Int, targetHeightWeight: Int): Size {
    return if (containerWidth * targetHeightWeight == containerHeight * targetWidthWeight) {
        // 比例一致, 直接返回容器尺寸
        Size(containerWidth, containerHeight)
    } else if (containerWidth * targetHeightWeight > containerHeight * targetWidthWeight) {
        // 容器宽度超出比例, 保持高度不变, 截取宽度
        Size(containerHeight * targetWidthWeight / targetHeightWeight, containerHeight)
    } else {
        // 容器高度超出比例, 保持宽度不变, 截取高度
        Size(containerWidth, containerWidth * targetHeightWeight / targetWidthWeight)
    }
}

fun calcInnerRect(containerWidth: Int, containerHeight: Int, targetWidthWeight: Int, targetHeightWeight: Int): Rect {
    return if (containerWidth * targetHeightWeight == containerHeight * targetWidthWeight) {
        // 比例一致, 直接返回容器尺寸
        Rect(0, 0, containerWidth, containerHeight)
    } else if (containerWidth * targetHeightWeight > containerHeight * targetWidthWeight) {
        // 容器宽度超出比例, 保持高度不变, 截取宽度
        val targetWidth = containerHeight * targetWidthWeight / targetHeightWeight
        val targetHeight = containerHeight
        Rect((containerWidth - targetWidth) / 2, 0, (containerWidth + targetWidth) / 2, containerHeight)
    } else {
        // 容器高度超出比例, 保持宽度不变, 截取高度
        val targetWidth = containerWidth
        val targetHeight = containerWidth * targetHeightWeight / targetWidthWeight
        Rect(0, (containerHeight - targetHeight) / 2, containerWidth, (containerHeight + targetHeight) / 2)
    }
}