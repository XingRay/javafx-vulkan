...
public class Color {
    ...
    @NonNull
    @Size(min = 4, max = 5)
    private final float[] mComponents = new float[4];
...
//    }
    ...
    public static void RGBToHSV(
            @IntRange(from = 0, to = 255) int red,
            @IntRange(from = 0, to = 255) int green,
            @IntRange(from = 0, to = 255) int blue,  float hsv[]) {
        if (hsv.length < 3) {
     ...
     */
    @ColorInt
    public static int HSVToColor(@IntRange(from = 0, to = 255) int alpha, float hsv[]) {
        if (hsv.length < 3) {
            throw new RuntimeException("3 components required for hsv");
        }
